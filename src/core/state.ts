...
function sanitizeConfig(raw: unknown): PluginConfig {
    ...
    if (typeof raw.farmServerUrl === 'string') out.farmServerUrl = raw.farmServerUrl;
    if (typeof raw.loginTimeout === 'number') out.loginTimeout = raw.loginTimeout;
...
class PluginState {
    ...
    /** 用户登录会话（简化存储） */
    userSessions = new Map<string, { isLoggedIn: boolean; gid: number; name: string; level: number; gold: number; exp: number }>();
