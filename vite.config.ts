...
// 依赖排除（如有外部依赖需排除，在此添加）
const external: string[] = ['ws', 'bufferutil', 'utf-8-validate'];
    ...
    return {
        ...
        writeBundle() {
            try {
                ...
                // 5. 复制 proto 目录（protobuf 定义文件）
                const protoSrc = resolve(__dirname, 'proto');
                if (fs.existsSync(protoSrc)) {
                    copyDirRecursive(protoSrc, resolve(distDir, 'proto'));
                    console.log('[copy-assets] (o\'v\'o) 已复制 proto 目录');
                }
